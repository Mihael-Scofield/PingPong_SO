...
void tratadorTemporizador() {
    ...
    if (tarefaAtual->ehTarefaUsuario == 0 || tarefaAtual->id == 0) { // Guarda de seguranca, apenas preempta tarefa de usuarios
        if (tarefaAtual->quantum == 0) { // Guarda para previnir preempcoes antes da hora
            if (estaPreemptando == -1) { // Guarda para previnir preempcoes em momentos indesejados
                estaPreemptando = 0;
                // Tratamento do tempo de processamento agora vai para o yield
