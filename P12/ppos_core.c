...
#include <signal.h>
#include <sys/time.h>
#include <string.h> // P12 - Para usar memcpy
...
int mqueue_msgs (mqueue_t *queue) {
    /* Guarda de seguranca */
    if ( (queue->filaDestruida == 1) || (queue == NULL) ){
        return -1;
    }

    // Basta retornar o ultimo id calculado, ja diz para nos quantas mensagens esperamos ter
    return (queue->idMensagem);
}

int mqueue_create (mqueue_t *queue, int max, int size) {
    /* Guardas de verificacao */
    if (queue == NULL) {
        fprintf(stderr, "mqueue_create: Fila passada para criacao ja foi criada. \n");
        return -1;
    }
    if (max < 1) {
        fprintf(stderr, "mqueue_create: Numero de mensagens maximo passado foi muito pequeno.\n");
        return -1;
    }
    if (size < 1) {
        fprintf(stderr, "mqueue_create: Tamanho da mensagem passado foi muito pequeno.\n");
        return -1;
    }

    /* Criacao propriamente dita */
    #ifdef debug
        printf("Criando nova fila de mensagens com maximo de %d e tamanho de mensagens de %d", max, size);
    #endif

    // Ordem da struct
    queue->maxMensagens = max;
    queue->tamanhoMensagens = size;
    queue->primeiroElemento = 0;
    queue->idMensagem = 0;
    queue->filaDestruida = 0;
      
    // Semaforos necessarios para o buffer circular
    sem_create(&queue->semaforoItens, 0);
    sem_create(&queue->semaforoLugares, max);
    sem_create(&queue->semaforoBuffer, 1);

    queue->buffer = malloc(max * sizeof(void*));
    for (int i = 0; i < max; i++){
        queue->buffer[i] = malloc(size);
    }        
...
int mqueue_recv (mqueue_t *queue, void *msg) {
    // Nao deve ler se a fila foi destruida, esta quebrada, ou seu primeiro elemento nao esta presente
    if ( (queue->filaDestruida == 1) ||
         (queue == NULL) ||
         (queue->buffer[queue->primeiroElemento] == NULL) ) {
        return -1;
    }
    
    /* Recebimento da mensagem em si */
    // Baixa semaforos para poder efetuar a leitura segura
    sem_down(&queue->semaforoItens);
    sem_down(&queue->semaforoBuffer);   

    // Uso de memcpy por recomendacao do professor
    // Usado o primeiro elemento para sabermos a partir de onde comecar a leitura, baseado no tamanho das mensagens dessa fila
    memcpy(msg, queue->buffer[queue->primeiroElemento], queue->tamanhoMensagens);
    // Atualiza o primeiro elemento, agora que a mensagem foi lida
    queue->primeiroElemento = (queue->primeiroElemento + 1) % queue->maxMensagens;    
    queue->idMensagem--; // IMPORTANTE: RETIRAR UM ID!!!!!!!
    
    sem_up(&queue->semaforoBuffer); // Ja podemos liberar o Buffer
    sem_up(&queue->semaforoLugares);

    return 0;
}

/* Funcao analoga ao recebimento */
int mqueue_send (mqueue_t *queue, void *msg) {
    ...
    // Baixa semaforos para poder efetuar a escrita de maneira segura
    sem_down(&queue->semaforoLugares);
    sem_down(&queue->semaforoBuffer);
  
    // Controle do indice
    // Pegamos o primeiro elemento e somamos com o ultimo id usado
    int indiceMensagem = ( (queue->primeiroElemento + queue->idMensagem) % queue->maxMensagens);

    // Uso de memcpy por recomendacao do professor
    memcpy(queue->buffer[indiceMensagem], msg, queue->tamanhoMensagens);
    queue->idMensagem++; // IMPORTANTE: ATUALIZAR O ID DE MENSAGEM!!!!!!!!!!!!!

    sem_up(&queue->semaforoBuffer); // Ja podemos liberar o buffer
    sem_up(&queue->semaforoItens);
...
int mqueue_destroy (mqueue_t *queue) {
    ...
    /* Destruicao da fila e dos semaforos */
    queue->filaDestruida = 1;
    sem_destroy(&queue->semaforoBuffer);
    sem_destroy(&queue->semaforoLugares);
    sem_destroy(&queue->semaforoItens);
    queue = NULL; // Perder o para garantir que caiamos nas guardas de seguranca

    return 0;
}

/* P10 - Consequências do P11 - Talvez fiquem para uso futuro, ainda nao sao necessarias */
